...
class Executor:
    ...
    def __init__(self):
        ...
        # 获取 MCP 管理器
        self.mcp_manager = get_mcp_manager(use_local=True)
        logger.info("Executor 初始化完成 (MCP 模式)")
...
"""
        ...
        # 使用 MCP 工具列表格式
        system_prompt = EXECUTOR_SYSTEM_PROMPT.format(available_tools=tools_desc)
        ...
        try:
            ...
            logger.info(f"[会话 {session_id}] Executor 决策: action_type={output.action_type}")
